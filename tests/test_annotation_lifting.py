...
from canopy import Annotations, Adat
...
def build_annotation_example():
    s = StringIO('SeqId,SomaId,Plasma Scalar v4.0 to v4.1\n54321-21,SL054321,0.8\n12345-12,SL012345,1.1\n')
...
def build_good_example_adat():
    rfu_data = [[1, 2], [4, 5]]
    col_metadata = {'SeqId': ['12345-12', '54321-21'], 'SeqIdVersion': ['1', '2'], 'ColCheck': ['PASS', 'FLAG']}
...
def build_example_adat_with_extra_somamers():
    ...
    col_metadata = {'SeqId': ['12345-12', '54321-21', '23456-78'], 'SeqIdVersion': ['1', '2', '3'], 'ColCheck': ['PASS', 'FLAG', 'FLAG']}
    row_metadata = {'PlateId': ['A12', 'A12'], 'Barcode': ['SL1234', 'SL1235']}
    header_metadata = {
        'AdatId': '1a2b3c',
        '!ProcessSteps': 'Raw RFU, Hyb Normalization, medNormInt (SampleId), plateScale, Calibration, anmlQC, qcCheck, anmlSMP',
...
class AdatLiftingTestCase(TestCase):
    ...
    def test_lifting_correct(self):
        ...
        self.assertTrue(lifted_adat.header_metadata['!ProcessSteps'].endswith(', Annotation Lift (v4.0 to v4.1)'))
    ...
    def test_orig_adat_unmodified(self):
        ...
        self.assertFalse(self.adat.header_metadata['!ProcessSteps'].endswith(', Annotation Lift (v4.0 to v4.1)'))
    ...
    def test_lifting_matrix_error(self):
        ...
        self.assertEqual(e_msg, 'Unsupported matrix: "NotSupportedMatrix". Supported matrices: Plasma.')
    ...
    def test_lifting_assay_version_error(self):
        ...
        with self.assertRaises(AnnotationsLiftingError) as cm:
            self.an.lift_adat(self.adat)
        e_msg = str(cm.exception)
        self.assertEqual(e_msg, 'Unsupported lifting from: "NotSupportedVersion". Supported lifting: from "v4.0" to "v4.1".')
    ...
    def test_lifting_to_space_error(self):
        ...
        e_msg = str(cm.exception)
        self.assertEqual(e_msg, 'Unsupported lifting from "v4.0" to "NotSupportedVersion". Supported lifting: from "v4.0" to "v4.1".')
    ...
    def test_analyte_mismatch(self):
        ...
        with self.assertRaises(AnnotationsLiftingError) as cm:
            self.an.lift_adat(self.adat)
        e_msg = str(cm.exception)
        self.assertEqual(e_msg, 'Unable to perform lifting due to analyte mismatch between adat & annotations. Has either file been modified?')

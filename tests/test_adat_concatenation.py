...
class ConcatRowsTest(TestCase):
    ...
    def test_row_metadata_mismatch(self):
        rfu_data = [[5, 6, 7], [6, 5, 4]]
        col_metadata = {'SeqId': ['A', 'B', 'C'], 'ColCheck': ['PASS', 'PASS', 'FLAG']}
        row_metadata = {'PlateId': ['A13', 'A13'], 'Barcode': ['SL1236', 'SL1237'], 'ExtraData': ['foo', 'bar']}
        header_metadata = {'AdatId': '1a2b3d', '!AssayRobot': 'Tecan2', 'RunNotes': 'run note 2'}
...
class SmartConcatTestCase(TestCase):
    ...
    def test_smart_adat_concat_header_meta(self):
        ...
        expected_header = {
            'AdatId': None, 
            '!AssayRobot': [{'adat_ids': ['stuff_A12'], 'value': 'Tecan1, Tecan2'}, {'adat_ids': ['morestuff_A13', 'evenmorestuff_A14'], 'value': 'Tecan2'}],
            'RunNotes': [{'adat_ids': ['stuff_A12'], 'value': 'run note 1'}, {'adat_ids': ['morestuff_A13', 'evenmorestuff_A14'], 'value': 'run note 2'}],
            '!Title': [{'adat_ids': ['stuff_A12'], 'value': 'stuff'}, {'adat_ids': ['morestuff_A13'], 'value': 'morestuff'}, {'adat_ids': ['evenmorestuff_A14'],'value': 'evenmorestuff'}]
        }
    ...
    def test_smart_adat_warnings(self):
        ...
        expected_warnings_regex = r'(Adding column to adat: \w+)|(Removing seqIds from \w{3}: \w, \w)'
        for rec in records:
    ...
    def test_smart_concat_adat_col_overwrite(self):
        ...
        self.assertEqual(list(concat_adat.columns.get_level_values('SeqIdVersion')), ['2', '3', '4'])

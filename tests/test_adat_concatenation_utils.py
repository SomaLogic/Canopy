...
class AdatColMetaReplaceTestCase(TestCase):
    def setUp(self):
        ...
        adat0 = Adat.from_features(rfu_data, row_metadata, col_metadata, header_metadata)
    ...
    def test_col_meta_replaced(self):
        ...
        meta_replaced_adats = adat_concatenation_utils.convert_somamer_metadata_to_source(self.adats, self.adats[0])
        pd.testing.assert_frame_equal(self.adats[0], meta_replaced_adats[0])
        pd.testing.assert_frame_equal(truth_meta_replaced_adat_1, meta_replaced_adats[1])
    ...
    def test_missing_col_meta_warns(self):
        with pytest.warns(UserWarning, match=r'Standard column, \w+, not found in column metadata. Continuing to next.'):
            adat_concatenation_utils.convert_somamer_metadata_to_source(self.adats, self.adats[0])
    ...
    def test_seq_ids_mismatch_error(self):
        ...
        with self.assertRaises(AdatMetaError):
            adat_concatenation_utils.convert_somamer_metadata_to_source(self.adats + [mismatch_seq_id_adat], self.adats[1])
...
class AdatHeaderMetaMergeTestCase(TestCase):
    def setUp(self):
        rfu_data = [[1, 2, 3], [4, 5, 6]]
        col_metadata = {'SeqId': ['A', 'B', 'C'], 'SeqIdVersion': ['1', '2', '3'], 'ColCheck': ['PASS', 'FLAG', 'FLAG']}
        row_metadata = {'PlateId': ['P0012345', 'P0012345'], 'Barcode': ['SL1234', 'SL1235']}
        header_metadata = {}
        self.adat0 = Adat.from_features(rfu_data, row_metadata, col_metadata, header_metadata)

        rfu_data = [[5, 6, 7], [6, 5, 4]]
        col_metadata = {'SeqId': ['A', 'B', 'C'], 'SeqIdVersion': ['1', '2', '3'], 'ColCheck': ['PASS', 'PASS', 'FLAG']}
        row_metadata = {'PlateId': ['Set 01', 'Set 02'], 'Barcode': ['SL1236', 'SL1237']}
        header_metadata = {}
    ...
    def test_get_adat_ids_v3(self):
        self.adat1.header_metadata = {
            '!AssayVersion': 'V4', '!AssayRobot': 'Fluent 2', 'PlateTailPercent_V4-19-021_Set_01': '2.56', '!Title': 'V4-19-021',
        }
    ...
    def test_get_adat_ids_v4(self):
        self.adat0.header_metadata = {
            '!AssayVersion': 'V4', '!AssayRobot': 'Fluent 1', 'PlateTailPercent_P0012345': '1.25', '!Title': 'P0012345',
        }
    ...
    def test_header_metadata_adat_id_converted(self):
        ...
        adat_ids = ['V4-19-021_Set 01', 'V4-19-021_Set 02']
        mod_header_metadata = adat_concatenation_utils._convert_header_meta_to_contain_adat_id(header_metadata, adat_ids)
    ...
    def test_merge_subheader_match(self):
        ...
        to_be_merged_subheader = [{
            'adat_ids': ['P0012345'],
            'value': 'V4'
        }]
        merged_subheader = adat_concatenation_utils._merge_subheader(master_subheader, to_be_merged_subheader)
    ...
    def test_header_simplify(self):
        ...
        simplified_header = adat_concatenation_utils._simplify_header_metadata(adat_id_header)
    ...
    def test_adat_header_merge(self):
        ...
        adat2.header_metadata = {
            '!AssayVersion': 'V4', '!AssayRobot': 'Fluent 1', 'PlateTailPercent_V4-19-025_Set_01': '3.5', '!Title': 'V4-19-025',
        }
        modified_header_metadata_adats = adat_concatenation_utils.robust_merge_adat_headers([self.adat0, self.adat1, adat2])
...
class RowMetaMergeTestCase(TestCase):
    ...
    def test_row_meta_merge_simple(self):
        ...
        ordered_merged_meta = adat_concatenation_utils.order_merge_row_meta_names([meta0, meta1, meta2])
...
class RowMetaAdatMergeTestCase(TestCase):
    def setUp(self):
        ...
        rfu_data = [[5, 6, 7], [6, 5, 4]]
        col_metadata = {'SeqId': ['A', 'B', 'C'], 'SeqIdVersion': ['1', '2', '3'], 'ColCheck': ['PASS', 'PASS', 'FLAG']}
        row_metadata = {'PlateId': ['A13', 'A13'], 'Barcode': ['SL1236', 'SL1237'], 'HybControlNormScale': ['1.2', '5'], 'RowCheck': ['PASS', 'PASS']}
        header_metadata = {'AdatId': '1a2b3d', '!AssayRobot': 'Tecan2', 'RunNotes': 'run note 2'}
        self.adat1 = Adat.from_features(rfu_data, row_metadata, col_metadata, header_metadata)

        rfu_data = [[8, 9, 1], [1, 4, 8]]
        col_metadata = {'SeqId': ['A', 'B', 'C'], 'SeqIdVersion': ['1', '2', '3']}
        row_metadata = {'PlateId': ['A14', 'A14'], 'Barcode': ['SL1238', 'SL1239'], 'Stuff': ['foo', 'bar']}
        header_metadata = {'AdatId': '1a2b3d', '!AssayRobot': 'Tecan2', 'RunNotes': 'run note 2'}
    ...
    def test_merge_norm_data(self):
        with pytest.warns(UserWarning, match=r'Adding column to adat: \w+'):
            new_adats = adat_concatenation_utils.unify_row_meta_column_names([self.adat0, self.adat1])
    ...
    def test_merge_three_adats(self):
        with pytest.warns(UserWarning, match=r'Adding column to adat: \w+'):
            new_adats = adat_concatenation_utils.unify_row_meta_column_names([self.adat0, self.adat1, self.adat2])
        expected_row_names = ['PlateId', 'Barcode', 'Stuff', 'HybControlNormScale', 'RowCheck']
        for adat in new_adats:
            self.assertEqual(expected_row_names, list(adat.index.names))
...
class SeqIdInnerMergeTestCase(TestCase):
    ...
    def test_inner_merge_two_adats(self):
        mod_adats = adat_concatenation_utils.prepare_rfu_matrix_for_inner_merge([self.adat0, self.adat1])
        expected_seq_ids = ['A', 'B']
        for adat in mod_adats:
            self.assertEqual(expected_seq_ids, list(adat.columns.get_level_values('SeqId')))

    def test_inner_merge_two_adats_warning(self):
        with pytest.warns(UserWarning, match=r'Removing seqIds from \w{3}: \w{1}'):
            adat_concatenation_utils.prepare_rfu_matrix_for_inner_merge([self.adat0, self.adat1])

    @pytest.mark.filterwarnings('ignore:Removing seqIds from')
    def test_inner_merge_three_adats(self):
        mod_adats = adat_concatenation_utils.prepare_rfu_matrix_for_inner_merge([self.adat0, self.adat1, self.adat2])
        expected_seq_ids = ['A']
        for adat in mod_adats:
            self.assertEqual(expected_seq_ids, list(adat.columns.get_level_values('SeqId')))
    ...
    def test_inner_merge_three_adats_warning(self):
        with pytest.warns(UserWarning, match=r'Removing seqIds from \w{3}: \w{1}'):
            adat_concatenation_utils.prepare_rfu_matrix_for_inner_merge([self.adat0, self.adat1, self.adat2])

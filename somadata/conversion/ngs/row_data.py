...
def convert_ngs_row_data(
    ...
    out_levels['RunYieldDemux'] = [str(ctx.yield_demux) if ctx.yield_demux is not None else ''] * n_rows
    out_levels['RunYieldQ30Demux'] = [str(ctx.yield_q30_demux) if ctx.yield_q30_demux is not None else ''] * n_rows
    out_levels['RunQ30WeightedMean'] = [str(ctx.q30_weighted_mean) if ctx.q30_weighted_mean is not None else ''] * n_rows
